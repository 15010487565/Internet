...
//如果同时有好多线程 同时去调用getInstance()方法  就可能会出现一些创建（new）多个BlackDao的现象  所以要加上synchronized
     ...
     * @param number
     */
    public void addBlackNum(String userId, String name, String nick, String headUrl, String number) {

        //获得一个可写的数据库的一个引用
        SQLiteDatabase db = dbHelper.getWritableDatabase();

        ContentValues values = new ContentValues();
        values.put("userId", userId);// KEY 是列名，vlaue 是该列的值
        ...
        Cursor cursor = db.rawQuery("SELECT * FROM black_num WHERE userId=?", new String[]{userId});
     ...
     * @param userId
     */
    public void deleteBlackNum(String userId) {
//		？?dad?asd?？sad?asdasdasd?
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        //表名  删除的条件
        db.delete(table_black_num, "userId = ?", new String[]{userId});
     ...
     * @param userId
     */
    public void updateBlackNumMode(String userId, String name, String nick, String headUrl) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put("userId", userId);// KEY 是列名，vlaue 是该列的值
        values.put("name", name);
        ...
        values.put("headUrl", headUrl);
//        values.put("number", number);
        db.update(table_black_num, values, " userId = ?", new String[]{userId});

    }

    public BlackNumBean rawQuery(String userId) {
